...
Next, specify and fit simple linear regression models using `fable::TSLM()`, and examine the model fit statistics. 
    ...
    ## 1 mod_temp   0.0332         0.0245  0.00416    3.78    0.0544     2    149. -610. -610. -602.
    ## 2 mod_prec…  0.000797      -0.00829 0.00430    0.0877  0.768      2    147. -606. -606. -598.
    ## 3 mod_temp…  0.0333         0.0156  0.00420    1.88    0.158      3    149. -608. -608. -597.
    ## # ℹ 4 more variables: CV <dbl>, deviance <dbl>, df.residual <int>, rank <int>
...
What does the content of the submission look like?
    ...
    ## 1 2022-01-01 OSBS    1         bet_abund_example_tslm_… ensem… abundan…    -0.0737 neon4cast 
    ## 2 2022-01-01 OSBS    2         bet_abund_example_tslm_… ensem… abundan…     0.0263 neon4cast 
    ## 3 2022-01-01 OSBS    3         bet_abund_example_tslm_… ensem… abundan…     0.0639 neon4cast 
    ## 4 2022-01-01 OSBS    4         bet_abund_example_tslm_… ensem… abundan…    -0.0266 neon4cast 
    ## 5 2022-01-01 OSBS    5         bet_abund_example_tslm_… ensem… abundan…    -0.0512 neon4cast 
    ## 6 2022-01-01 OSBS    6         bet_abund_example_tslm_… ensem… abundan…     0.0230 neon4cast 
...
For immediate feedback, we can use the targets data from 2022 to score our forecast for the 2022 field season at OSBS. 
    ...
    ## # A tibble: 6 × 17
    ##   model_id    reference_datetime site_id datetime   family variable observation   crps   logs
    ##   <chr>       <chr>              <chr>   <date>     <chr>  <chr>          <dbl>  <dbl>  <dbl>
    ## 1 bet_abund_… 2022-01-01         OSBS    2022-04-04 sample abundan…      0.102  0.0308 -1.49 
    ## 2 bet_abund_… 2022-01-01         OSBS    2022-04-18 sample abundan…      0.188  0.0971 -0.286
    ## 3 bet_abund_… 2022-01-01         OSBS    2022-04-25 sample abundan…      0.0877 0.0185 -1.97 
    ## 4 bet_abund_… 2022-01-01         OSBS    2022-05-02 sample abundan…      0.0857 0.0111 -2.08 
    ## 5 bet_abund_… 2022-01-01         OSBS    2022-05-16 sample abundan…      0.0786 0.0294 -1.15 
    ## 6 bet_abund_… 2022-01-01         OSBS    2022-05-30 sample abundan…      0.133  0.0299 -1.46 
    ## # ℹ 8 more variables: mean <dbl>, median <dbl>, sd <dbl>, quantile97.5 <dbl>,
    ## #   quantile02.5 <dbl>, quantile90 <dbl>, quantile10 <dbl>, horizon <drtn>
...
![ ](https://raw.githubusercontent.com/NEONScience/NEON-Data-Skills/main/tutorials/R/biodiversity/beetle-forecasting/rfigs/visualize forecasts to raw target data-1.png)
