...
## 4. Additional Plots to Visualize Diversity:
Make some diversity plots to show number of species being captured.

```{r plotDiversity, message=FALSE, warning=FALSE}
#NOTE TO SELF - DOUBLECHECK THE CAPTURES INPUT DATAFRAME
TaxDat<-captures %>% 
  mutate(siteID = substr(plotID,1,4)) %>% 
  group_by(taxonID, siteID) %>% summarise(tot=n())

TaxPlot<-ggplot(TaxDat[which(TaxDat$tot>0),], aes(x=taxonID, y=tot, fill=taxonID)) + 
