...
It is recommended that NEON data users have a NEON Data Portal API token set as an environment variable. See <a href="https://www.neonscience.org/resources/learning-hub/tutorials/neon-api-tokens-tutorial" target="_blank">this tutorial.</a>
for instructions on obtaining a NEON API token.
...
data by setting the `include.provisional` argument to 'true' in the
...
```{r R-download, results="hide", message=FALSE, warning=FALSE}
...
```{python p-download-hidden, eval=FALSE, include=FALSE}
...
def read_files_to_dict(directory):
    ...
    return data
csd_py = read_files_to_dict("C:/Users/nickerson/Downloads/python-files/filesToStack00130/stackedFiles")
```

```{python p-download, eval=TRUE, results="hide", message=FALSE, warning=FALSE}
# Download continuous discharge data across the lifetime of a site
...
```{r R-files, results="hide", message=FALSE, warning=FALSE}
...
```{python p-files, results="hide", message=FALSE, warning=FALSE}
# Get all file names in the download package
csd_py.keys()
```
...
```{r R-tables, results="hide", message=FALSE, warning=FALSE}
...
```{python p-tables, results="hide", message=FALSE, warning=FALSE}
# Print the first 5 records in each of the time series tables to view structure
print("First 5 rows of csd_continuousDischarge")
...
```{r R-variables, results="hide", message=FALSE, warning=FALSE}
...
```{python p-variables, results="hide", message=FALSE, warning=FALSE}
# View variables file to understand data table structure
...
This data product was partially updated in 2026 from uncorrected 1 minute 
values to corrected 15-min averages. Until these updates are fully applied
...
and to convert quality flags to either 1 (flagged) or NA.

#### R

```{r R-aggregate, results="hide", message=FALSE, warning=FALSE}
...
```{python p-aggregate, results="hide", message=FALSE, warning=FALSE}
# Round, group, and summarize data, and update column names to match csd_15_min
...
```{r R-join, results="hide", message=FALSE, warning=FALSE}
...
```{python p-join, results="hide", message=FALSE, warning=FALSE}
...
```{r R-plot-stage, results="hide", message=FALSE, warning=FALSE, warning=FALSE}
...
```{python p-plot-stage, results="hide", message=FALSE, warning=FALSE, warning=FALSE}
...
```{r R-plot-discharge, results="hide", message=FALSE, warning=FALSE, warning=FALSE}
...
```{python p-plot-discharge, results="hide", message=FALSE, warning=FALSE, warning=FALSE}
