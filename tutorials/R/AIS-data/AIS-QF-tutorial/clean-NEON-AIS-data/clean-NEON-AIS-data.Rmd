...
title: "Clean NEON Aquatic Instrument Data"
...
languageTool: R, API
code1: R/clean-neon-ais-data/clean-neon-ais-data.R
tutorialSeries:
...
```{r download-data-waq, results='hide', warning=FALSE}
...
```{r subset-waq-horizontalPosition}
...
a specified minimum and maximum are set to NA. Fr example, Dissolved Oxygen 
...
```{r download-data-nsw, results='hide', warning=FALSE}
...
```{r check-nsw-horizontalPosition}
...
Therefore, we no not need to subset this dataset like we did for Water quality.
...
We can then rep-lot the shifted data.
