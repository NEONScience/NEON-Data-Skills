...
## ----plot-wqual, fig.cap= "Line plot of dissolved oxygen in mg/L measured at the upstream(blue) and downstream(red) stations of the Pringle Creek site."----
# plot
wqual <- ggplot() +
...
## ----plot-fdom-ucert, fig.cap= "Line plot of fDOM(QSU) with expected uncertainty from the downstream station of Pringle Creek."----
# plot
fdomUcert <- ggplot() +
	geom_line(data = waq_down, 
...
## ----challenge-plot-nsw, fig.cap= "Nitrate in surface water(uM) from the downstream station of Pringle Creek. Note that there is missing data from February 18th through February 24th."----
# plot
...
## ----omit-finalqf, fig.cap= "Line plots of fDOM data that received a quality flag of zero(top), and data that received a data quality flag of one(bottom). Note how the bottom plot has many spikes in the data and were appropriately given a flag value of one."----
...
## ----view-absQF, fig.cap= "Line plots of fDOM absorbance quality flag (fDOMAbsQF) values that received a quality flag of -1,0,1, and 2.  Note how the plot corresponding to values flagged as 1 correspond to the same time frame of data missing in the surface water nitrate generated earlier."----
ggplot2::ggplot(data = waq_down, 
...
## ----look-at-n03-again,  fig.cap= "Nitrate in surface water(uM) from the downstream station of Pringle Creek. Note that there is missing data from February 18th through February 24th."----
plot_NSW


## ----ignore-absQF,  fig.cap= "Line plots of fDOM data where a custom quality flag has been generated by omitting the fDOMAbsQF. Note the increase in available data using the custom quality flag aggregation that ignored fDOMAbsQF."----
...
## ----plot-eos-waq, Scatter plot of specific conductance (uS/cm) and elevation (m) from Pringle Creek. Specific conductance (uS/cm) is on the Y-axis and elevation (m) on the X-axis. A new data set of 30 minute aggregated water quality data was generated to match the measurement interval of surface water elevation."----
