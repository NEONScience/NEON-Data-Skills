...
description: Tutorial for downloading NEON AIS data using the neonUtilities
package and then exploring and understanding the downloaded data including 
(1) separating data by sensor locations using the HOR variable, 
(2) interpreting quality flags, and
(3) aggregating data by time.

dateCreated: '2020-05-19'
authors: Bobby Hensley, Megan Jones, Guy Litt
contributors: 
estimatedTime: Three hours
...
your data, neonUtilities will ignore this line of code. We set  
...
```{r download-data-nsw, echo=FALSE}
...
```{r download-data-eos, echo=FALSE}
...
```{r challenge-explore-nsw, echo=FALSE}
...
waq_mean_30min_down <- waq_down %>% # pass the downstream data frame to the next function
              ...
              #  by the the mean, for all data columns provided in waq_strm_cols
              dplyr::group_by(roundedTime) %>% dplyr::summarise_at(vars(waq_strm_cols), mean, na.rm = TRUE)

# Aggregate waq's quality flags to 30 minutes by selecting the maximum quality flag value
waq_finalqf_30min_down <- waq_down %>% 
              padr::thicken(interval = "30 min",by = "endDateTime",colname = "roundedTime") %>%
              dplyr::group_by(roundedTime) %>% dplyr::summarise_at(vars(waq_final_qf_cols), max, na.rm = TRUE)


# Merge water quality's time-averaged data with the time-aggregated quality flags:
waq_30min_down <- base::merge(waq_mean_30min_down,waq_finalqf_30min_down,by = "roundedTime")
View(waq_30min_down)
