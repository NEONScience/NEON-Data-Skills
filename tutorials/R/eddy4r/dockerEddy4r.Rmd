...
[CKL: I think there is another step here, getting access to eddy4r. I don't 
think a new account would automatically be able to access it?
DD: The eddy4r repo will be public once the manuscript is published, but for now you do need to be given access]
...
Note: If run just the word ``docker`` you should see a whole bunch of lines showing the different options available with ``docker``. Alternatively you can test your installation by running the following:
...
If either of these steps work, you are ready to go on to access the eddy4R-Docker image that houses the suite of eddy4R R 
...
The second command `docker run -d -p 8787:8787 stefanmet/eddy4r:0.2.0` will [DD: `eddy4r:0.2.0` will need to be updated to align with upcoming manuscript and public release or you can grab latest with `eddy4r:latest`]
...
you are running locally on a .[CKL: something missing here?] [DD: PC, also it is important to specify that the second port ID must always be 8787 as this is the port that RStudio uses to communicate. The first one can be any open port on your machince] If you are running 
many containers or on a shared network, you may need to specify a different port.
...
[MJ:what does this mean; DD: If you are not grabbing data from an ECS bucket or other web-based storage system. Then, it may be necessary to mount a local filesytem to the the docker container to access your data files.] If data are not directed from/to cloud hosting, a 
...
[CKL: I think this means if you want to use data from your local computer, 
instead of data in the cloud, you can mount a local directory into the 
container. I didn't try it. DD: Correct]
...
[CKL: not sure. I did it without the /auth-sign-in and worked fine.]
[DD: Yeah, no difference. If you type it in it will point you to the sign in if you haven't already signed into that container.]
...
containers can be found on the <a href="https://github.com/rocker-org/rocker/wiki/Using-the-RStudio-image" target="_blank">rocker-org/rocker website</a> and the <a href="" target="_blank">eddy4RWiki pages</a> [MJ:where is this, I can't find it].[CKL: I think this is 
referring to the extensive wiki in the NEON-FIU-algorithm repo, which 
is restricted access. For now we should probably drop the reference, 
but longer term should talk to Dave and Stefan about what documentation 
will be turned public in the future.]
[DD: Yeah, the NEON-FIU-algorithm Wiki is currently being revamped and will go public with the code and manuscript. So, eventually the link will be https://github.com/NEONScience/eddy4R/wiki]
