...
It will be in the form DP#.#####.###. For this tutorial, we'll use three data
product: 
...
We can see that there are four objects in the downloaded nitrate data. One 
...
`horizontalPosition` variable in the data tables to describes which data a sensor 
...
* 130 and 140: sensors mounted in the **lottoral zone** of lakes
...
surface water, comparing as many sensor as are available in the data set.

</div>

```{r challenge-code-extent-crs, echo=FALSE}
# create DataFrame
nitrate1 <- nitrate$NSW_15_minute

# which sensor locations?
unique(nitrate1$horizontalPosition)

# plot
NO3plot <- ggplot(nitrate1, aes(startDateTime, surfWaterNitrateMean)) +
