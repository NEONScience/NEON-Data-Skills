...
phe <- loadByProduct(dpID = "DP1.10055.001", site="JORN", 
										 startdate = "2018-07", enddate="2018-12", 
										 check.size = F)
...
those distinct value and not join on them. Therefore, we can rename the common 
...
Our `editedDate` and `date` columns are stored as a `character` class. We need 
to convert it to a date class. The `as.Date()` function in base R will do this. 
...
str(status_noD$dateStat)

status_noD$editedDateStat <- as.Date(status_noD$editedDateStat)
str(status_noD$editedDateStat)
...
write.csv( phe_1sp_2016 , file="NEONpheno_LITU_Leaves_SCBI_2016.csv", row.names=F)
