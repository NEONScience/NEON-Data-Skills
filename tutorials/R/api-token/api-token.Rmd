...
code with an alias. There are a few ways to do this. Let's start by simply 
assigning the token to a variable name.

```{r useTokenName, eval=FALSE, comment=NA}
...
Of course, this is visible to anyone you share your code with, and when you 
close your R session, the `NEON_TOKEN` variable will be cleared, so you'd 
have to re-create the variable in every session. We'll show two options for 
doing this smoothly:
