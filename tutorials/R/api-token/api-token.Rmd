...
Now we'll use the `loadByProduct()` function to download data. For 
...
repository, you'll need to save your token locally, and only reference it in 
your code. There are a few ways to do this. Here, we'll assign the token to 
...
be cleared, so you'd have to re-run `Sys.setenv()` in every session. We'll 
show two options for doing this smoothly:

Option 1: Save the token in a file, and `source()` that file at the start of 
every script. This is simpler!

Option 2: Add the token to a `.Renviron` file. This is harder to set up 
initially, and you must be using RStudio for it to work, but once it's done, 
it's done globally, and it will work in every script you run.
