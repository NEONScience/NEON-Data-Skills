...
names(field_spectra)
```
We encourage looking at all of these variables to learn more about the data product, but you can find the actual data stored in the variables: `fsp_boutMetadata` (contains metadata information about this sampling bout), `fsp_sampleMetadata` (contains relevant metadata about the individual sample), and `fsp_spectralData` (contains information about the individual sample data), and `per_sample` (contains the actual wavelength and reflectance data). Next, we can merge the data and metadata into a single table as follows. 
...
FSP1 <- field_spectra$per_sample[which(field_spectra$per_sample$spectralSampleID=="FSP_RMNP_20200706_2043"),]
