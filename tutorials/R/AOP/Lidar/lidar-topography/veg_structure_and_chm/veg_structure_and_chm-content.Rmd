...
gdf = gpd.GeoDataFrame(
       vegloc, geometry=gpd.points_from_xy(vegloc.adjEasting, vegloc.adjNorthing))
gdf["geometry"] = gdf.geometry.buffer(0.9)
...
idx = np.isfinite(vegsub.height) & np.isfinite(CHMbufflist)
np.corrcoef(vegsub.height[idx], CHMbufflist[idx])[0,1]
...
valCHM10 = list(chm10.sample(tuple(zip(vegbin["easting10"]+5, vegbin["northing10"]+5))))
...
idx = np.isfinite(vegbin.height_max) & np.isfinite(CHM10list)
np.corrcoef(vegbin.height_max[idx], CHM10list[idx])[0,1]
...
filterCHM = list(chm.sample(tuple(zip(vegfil["adjEasting"], vegfil["adjNorthing"]))))
...
idx = np.isfinite(height.height) & np.isfinite(filCHMlist)
np.corrcoef(height.height[idx], filCHMlist[idx])[0,1]
...
### Use canopy position data {.tabset}

Finally, there is an alternative way to determine which trees are at 
the top of the canopy: it's in the vegetation structure data! There is 
a data field for `canopyPosition`. Let's subset to only the trees 
described as `"Full sun"`.
...
vegtop = vegsub.loc[vegsub.canopyPosition=="Full sun"]
topCHM = list(chm.sample(tuple(zip(vegtop["adjEasting"], vegtop["adjNorthing"]))))

fig, ax = plt.subplots()

ax.plot((0,50), (0,50), linewidth=1, color="black")
ax.scatter(vegtop.height, topCHM, s=1)
...
topCHMlist = np.array([c.tolist()[0] for c in topCHM])
idx = np.isfinite(vegtop.height) & np.isfinite(topCHMlist)
np.corrcoef(vegtop.height[idx], topCHMlist[idx])[0,1]

```

### {-}

Using all the data collected on the ground helps a lot! We left this 
option for last because in the early days of NEON data collection, 
the `canopyPosition` field was only recorded intermittently, so we 
may have discarded trees erroneously. But as time goes on, this 
dataset will continue to improve.
# CKL: rephrase this, and review data - do all individualIDs have a canopyPosition now?
...
returns from short-statured vegetation are difficult to distinguish 
