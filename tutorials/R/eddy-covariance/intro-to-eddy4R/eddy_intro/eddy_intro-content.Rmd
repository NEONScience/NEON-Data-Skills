---
title: Content for Introduction to working with NEON eddy flux data
...
flux data, using the `neonUtilities` R package or the `neonutilities` 
Python package. If you are new to NEON 
data, we recommend starting with a more general tutorial, such as the 
<a href="https://www.neonscience.org/download-explore-neon-data" target="_blank">Download and Explore tutorial</a>. 
Some of the functions and techniques described there will 
...
## 1. Setup and data download {.tabset}
...
Now load packages and API token.
...
Use the `zips_by_product()` function from the `neonutilities` package to 
download flux data from two sites and two months. The transformations 
and functions below will work on any time range and site(s), but two 
sites and two months allows us to see all the available functionality 
while minimizing download size.
...
## 3. Extract Level 4 data (Fluxes!) {.tabset}
...
To extract the Level 4 data from the HDF5 files and merge them into a 
single table, we'll use the `stack_eddy()` function from the `neonutilities` 
package.
...
Input the filepath you downloaded to using `zips_by_product()` earlier, 
including the `filestoStack00200` folder created by the function, and 
`dp04`:


```{python p-stack-dp04, results="hide"}
...
We now have an object called `flux`. It's a dictionary containing four 
tables: one table for each site's data, and `variables` and `objDesc` 
tables.
...
```{r download-par, results="hide", message=FALSE}
...
To get NEON PAR data, use the `load_by_product()` function from the 
`neonutilities` package. `load_by_product()` takes the same inputs as 
`zips_by_product()`, but it loads the downloaded data directly into the 
...
```{python p-download-par, results="hide", message=FALSE}
...
field. This field indicates the position on the tower, with 010 being the 
first tower level, and 020, 030, etc going up the tower.

### Join PAR to flux data {.tabset}
...
As noted above, `load_by_product()` automatically converts time stamps 
to a recognized date-time format when it reads the data. However, the 
field names for the time stamps differ between the flux data and the 
other meteorological data: the start of the averaging interval is 
`timeBgn` in the flux data and `startDateTime` in the PAR data.

Let's create a new variable in the PAR data and subset to the top of the 
tower.

```{python p-get-tower-top}
...
## 5. Vertical profile data (Level 3) {.tabset}
...
Note (Feb 17, 2026): There is a bug in the Python implementation of the Level 3 code. 
This section may not run correctly, we will update as soon as possible.

```{python p-stack-dp03, eval=FALSE, results="hide", message=FALSE}
...
## 6. Un-interpolated vertical profile data (Level 2) {.tabset}
...
```{python p-stack-dp02, results="hide", message=FALSE}
...
Using `stack_eddy()` to extract Level 1 data requires additional 
inputs. The Level 1 files are too large to simply pull out all the 
variables by default, and they include multiple averaging intervals, 
which can't be merged. So two additional inputs are needed:
...
Inputs to `var` can be any values from the `name` field in the table 
returned by `get_vars_eddy()`. Let's take a look at CO<sub>2</sub> and 
H<sub>2</sub>O, <sup>13</sup>C in CO<sub>2</sub> and <sup>18</sup>O in 
H<sub>2</sub>O, at 30-minute aggregation. Let's look at Harvard Forest 
for these data, since deeper canopies generally have more interesting 
profiles:
...
```{python p-stack-dp01, results="hide", message=FALSE}
...
`ggplot` is well suited to these types of data. We can plot CO<sub>2</sub> 
relative to height on the tower, with separate lines for each time interval.
...
```{python p-get-one-day, warning=FALSE}
...
Let's plot CO<sub>2</sub> 
relative to height on the tower, with separate lines for each time interval.

```{python p-plot-co2-profile, message=FALSE, warning=FALSE}
...
```{python p-plot-iso-profile, message=FALSE, warning=FALSE}
