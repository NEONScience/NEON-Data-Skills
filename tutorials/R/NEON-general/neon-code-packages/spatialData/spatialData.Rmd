...
We can use the easting and northing data to plot the locations of the 
individual trees.
...
Here we see a set of little boxes scattered across the landscape. 
We've plotted each individual tree as a `.`, but by plotting all 
the trees across the site, all we can see at this scale is the cluster 
of dots that make up each plot.
