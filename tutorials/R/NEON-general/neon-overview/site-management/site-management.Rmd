...
The `sim_locationVisitsAndAttributes` table provides a summary of annual visit totals, environmental context, and mitigation actions by location to support estimates of NEON sampling impact and location vulnerability. For TEAK, and all other terrestrial sites, each `namedLocation` has a record per year to the total unique visits per sampling plot. Additional fields describe environmental context, document location-specific mitigation measures (such as boardwalks or geoblocks used to minimize trampling), and report the number of sampling events that were cancelled because the location was deemed too vulnerable. Currently, TEAK has no additional mitigation structures, and one sampling bout at TEAK_013 was cancelled in 2025 due to vulnerability. Aquatic sites are summarized the total number of visits per site and do not provide the more specific environmental context fields. 

Let's take a closer look at `sim_eventData` which documents the external disturbance events.

``` {r view-data,sim_eventData }
...
The `sim_locationVisitsAndAttributes` package provides a `summarizeSIM` function that provides 3 summary tables by site, site and eventType, and site, eventType, and year. 

``` {r view-types, neonSiteMgmtEventData::summarizeSIM}
neonSiteMgmtEventData::summarizeSIM(df= sim_eventData)
```

The $summary table sums the number of records in the df and summarizes how many individual records, unique events (since a event thta is longer than one month with have mutiple records), and unique event types. 

Next, $eventTypeTable counts the number of unique eventIDs by eventType and site. The last table, adds in a year grouping as well.  At TEAK, there are over 100 records but most correspond to a long term grazing event. 
...
We can utilize the `sim_locationVisitsAndAttributes` package's `createSIMPolygon` function to create a polygon around the point locations for each event. The input is a data frame of one to many SIM events that has been downloaded from the NEON portal and run through `geoNEON::getLocTOS`.

``` {r plot-2019-add polygons}

#wildlife polygons
wf.SOAP.df <- fire.loc |>
  filter(siteID=="SOAP" & endDate<"2021-01-01" & eventType == "fire" & methodTypeChoice=="fire-wildfire") 

wf.SOAP.polygon <- neonSiteMgmtEventData::createSIMPolygon(df= wf.SOAP.df)

#transform the polygon from WGS84 (standard output) to utm zone 11N to match AOP tiles.  
#coordinate reference system (crs) EPSG values can be searched for on https://spatialreference.org/ref/epsg/
wf.SOAP.polygon.utm <- st_transform(wf.SOAP.polygon$sfObjects[[1]], crs = 32611)


#control burn polygons
cb.SOAP.df <- fire.loc |>
  filter(siteID=="SOAP" & endDate<"2021-01-01" & eventType == "fire" & methodTypeChoice=="fire-controlledBurn")

cb.SOAP.polygon <- neonSiteMgmtEventData::createSIMPolygon(df= cb.SOAP.df)

#transform the polygon from WGS84 (standard output) to utm zone 11N to match AOP tiles.  
#coordinate reference system (crs) EPSG values can be searched for on https://spatialreference.org/ref/epsg/
cb.SOAP.polygon.utm <- st_transform(cb.SOAP.polygon$sfObjects[[1]], crs = 32611)
...
The controlled burn polygon (dark red) is the smallest convex polygon around the listed locationID points. 
...
If helpful for further analysis, we export the polygons into a shapefile or kml using `sim_locationVisitsAndAttributes::writeExportFunction`.

``` {r export polygons}
#export the wildfire polygons as kmls
neonSiteMgmtEventData::writeExportFile(sfList = wf.SOAP.polygon$sfObjects, outDir = getwd(),
                                       outType = "kml")

#export the controlled burns polygons as shapefiles
neonSiteMgmtEventData::writeExportFile(sfList = cb.SOAP.polygon$sfObjects, outDir = getwd(),
                                       outType = "shapefile")

```

For more information and examples how how the `createSIMPolygon` works please see the vignette assoicated withe the package. 
devtools::install_github('NEONScience/neonSiteMgmtEventData', force = T,build_vignettes = TRUE)
