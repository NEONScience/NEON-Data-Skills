...
* **neonUtilities**: Basic functions for accessing NEON data in R
* **neonutilities**: Basic functions for accessing NEON data in Python
* **neonOS**: Functions for common data wrangling needs for NEON observational data in R. No equivalent in Python; the tutorial uses custom Python code in those sections.
* **terra**: Spatial data package; needed for working with remote sensing data in R.
...
```{python p-plot-par, eval=TRUE}

par30top = par30[par30.verticalPosition=="080"]
plt.plot(par30top.endDateTime, par30top.PARMean)
plt.show()
...
```{python p-plot-par-lower, eval=TRUE}

par30low = par30[par30.verticalPosition=="020"]
plt.plot(par30top.endDateTime, par30top.PARMean)
plt.plot(par30low.endDateTime, par30top.PARMean, 
         color="blue")
plt.show()
...
```{python p-run-loadByProduct-par, results="hide"}
...
see this by checking the names of the tables in 
`parlist`:
...
* `token`: Use a NEON API token for faster downloads. See <a href="https://www.neonscience.org/resources/learning-hub/tutorials/neon-api-tokens-tutorial" target="_blank">this tutorial</a> 
...
```{python p-13C-by-site, message=FALSE, warning=FALSE}

apl13C = apl_plantExternalLabDataPerSample[
         apl_plantExternalLabDataPerSample.analyte=="13C"]
plt.boxplot(apl13C["siteID"],
            apl13C["analyteConcentration"])
...
```{python p-plot-13C-by-tax, eval=TRUE}

apl13Cspp = apct[apct.analyte=="13C"]
plt.boxplot(apl13Cspp["scientificName"],
            apl13Cspp["analyteConcentration"])
...
byTileAOP("DP3.30015.001", site="WREF", year="2017",
          ...
          savepath="~/Downloads", check.size=F)
