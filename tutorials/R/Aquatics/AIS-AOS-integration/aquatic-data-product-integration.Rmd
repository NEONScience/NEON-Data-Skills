...
### Abun and Richness + Discharge Over Time
...
## Case Study 1: Hurricane-impacted streams: Examine relationships between discharge, sediment particle size distribution, and macroinvertebrate abundance/diversity
...
```{r highlight-fiona, out.width='100%', out.height='600px', fig.height=4, fig.width=6}
...
## Case Study 2: Macroinvertebrates in drying streams: visualize the interplay between abundance, richness, and flow within and across water years
...
assess the effect those hydrologic parameters have one macroinvertebrate 
communities within and across water years at a site.

```{r }
# Assign each record a water year in csd_15_min
...
# Assign water year and day-of-water-year to abundance summary
...
# Assign water year and day-of-water-year to richness summary
...
# Build one stacked multi-axis plot per site with cumulative flow, abundance, and richness
for(s in 1:length(siteList)){
  waterYears <- sort(unique(csd_15_min_summ$waterYear[csd_15_min_summ$siteID==siteList[s]]))

  # Compute fixed axis ranges for this site so all WY panels are directly comparable
...
## Case Study 4: Can relationships between sensor measurements and discrete water chemistry data be used to expand the temporal extent of ecologically-relevant analytes?
