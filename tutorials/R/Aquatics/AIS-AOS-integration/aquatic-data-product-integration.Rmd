...
variables_20120_dupCheck <- variables_20120%>%
  filter(!(table=="inv_taxonomyProcessed"
           &fieldName%in%c("vialID","vialCode","slideID","slideCode",
                           "referenceCollection","referenceCount",
                           "referenceID","referenceCode")))
inv_taxonomyProcessed_dups <- neonOS::removeDups(inv_taxonomyProcessed,
                                                 variables_20120_dupCheck)
cat("There are",
    sum(inv_fieldData_dups$duplicateRecordQF%in%c(1,2)),
...
sampler_habitat_summ <- inv_fieldTaxJoined%>%
  dplyr::distinct(siteID,habitatType)
...
inv_abundance_summ <- inv_fieldTaxJoined%>%
  ...
  dplyr::summarise(across(abun_M2_sum, 
...
csd_15_min%>%
  ggplot2::ggplot(aes(x=startDateTime,y=dischargeContinuous))+
  ggplot2::geom_line()+
  ggplot2::facet_wrap(~siteID,ncol = 1,scales = "free_y")+
...
```{r aos-ais-plot, out.width='100%', out.height='600px', fig.height=4, fig.width=8}
for(s in 1:length(siteList)){
  # begin the plot code
  AOS_AIS_plot <- csd_15_min%>%
    dplyr::filter(siteID==siteList[s])%>%
    plotly::plot_ly()%>%
    # add trace for continuous discharge
    plotly::add_trace(x=~startDateTime,y=~dischargeContinuous,
  ...
  htmlwidgets::saveWidget(plotly::as_widget(AOS_AIS_plot),paste0("~/",siteList[s],"_INV_CSD.html"))
}

```

<div class="figure" style="text-align: center">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/CUPE_INV_CSD.png" alt=" "  />
<p class="Discharge Timeseries and Macroinvertebrate Data At D04-CUPE">CAPTION</p>
</div>
...
```{r case study 1 inputs, include=FALSE}
siteList <- c("WLOU")
startWY <- 2022
endWY <- 2024
startWY_query <- paste0(startWY-1,"-10")
...
```{r plot-model-doc}
# create plot
fDOM_DOC_plot <- plotly::plot_ly(data=fDOM_DOC_join)%>%
...
# Save HTML plot to local documents folder
htmlwidgets::saveWidget(plotly::as_widget(fDOM_DOC_plot),paste0("~/",siteList,"_fDOM_DOC_model.html"))

```

<div class="figure" style="text-align: center">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/WLOU_fDOM_DOC_model.png" alt=" "  />
...
```{r case study 2 inputs, include=FALSE}
siteList <- c("CUPE","GUIL")
startWY <- 2022
endWY <- 2024
startWY_query <- paste0(startWY-1,"-10")
...
<div class="figure" style="text-align: center">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/fionaNEON.png" alt=" "  />
...
# Save HTML plot to local documents folder
htmlwidgets::saveWidget(plotly::as_widget(AOS_AIS_plot_CUPE_Fiona),"~/CUPE_INV_CSD_Fiona.html")
...
AOS_AIS_plot_GUIL_Fiona <- AOS_AIS_plot_GUIL%>%
  ...
  plotly::add_segments(x=as.POSIXct(fionaDate,tz="UTC"),
                       ...
                       line=list(color='red',dash='dash'))
# Save HTML plot to local documents folder
htmlwidgets::saveWidget(plotly::as_widget(AOS_AIS_plot_GUIL_Fiona),"~/GUIL_INV_CSD_Fiona.html")
...
# Save HTML plot to local documents folder
htmlwidgets::saveWidget(plotly::as_widget(AOS_AIS_plot_CUPE_Fiona),"~/CUPE_INV_CSD_GEO_Fiona.html")
...
# Save HTML plot to local documents folder
htmlwidgets::saveWidget(plotly::as_widget(AOS_AIS_plot_GUIL_Fiona),"~/_INV_CSD_GEO_Fiona.html")

```

<div class="figure" style="text-align: center">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/CUPE_INV_CSD_GEO.png" alt=" "  />
...
```{r case study 3 inputs, include=FALSE}
siteList <- c("KING","ARIK","SYCA")
startWY <- 2022
endWY <- 2025
startWY_query <- paste0(startWY-1,"-10")
...
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/KINGdry.png" alt=" "  />
...
<div class="figure" style="text-align: center" width="50%">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/ARIKdry.png" alt=" "  />
...
<div class="figure" style="text-align: center" width="50%">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/SYCAdry.png" alt=" "  />
...
```{r case study 3 wrangle}
# Assign each discharge record a water year
csd_15_min <- csd_15_min%>%
  dplyr::mutate(waterYear=ifelse(lubridate::month(startDateTime)>=10,
                                lubridate::year(startDateTime)+1,
                                lubridate::year(startDateTime)),
...
csd_15_min_summ <- csd_15_min%>%
  dplyr::arrange(siteID, waterYear, startDateTime)%>%
  dplyr::group_by(siteID,waterYear)%>%
  dplyr::mutate(cumulativeFlow=cumsum(dischargeContinuous),
...
inv_abundance_case2 <- inv_abundance_summ%>%
  dplyr::ungroup()%>%
  dplyr::mutate(waterYear=ifelse(lubridate::month(collectDate)>=10,
                                 lubridate::year(collectDate)+1,
                                 lubridate::year(collectDate)),
                waterYearStart=as.Date(paste0(waterYear-1,"-10-01")),
                dayOfWaterYear=as.integer(as.Date(collectDate)-waterYearStart)+1,
                plotDateWY=as.Date("1999-10-01") + (dayOfWaterYear-1),
                abun_M2_sum_se=dplyr::coalesce(abun_M2_sum_se,0))%>%
  dplyr::arrange(siteID,waterYear,habitatType,dayOfWaterYear)
...
for(s in 1:length(siteList)){
  ...
  for(wy in 1:length(waterYears)){
    ...
    cumsumPlot <- plotly::plot_ly(data=csd_15_min_summ%>%
                                    dplyr::filter(siteID==siteList[s]
                                                  &waterYear==waterYears[wy]))%>%
      # Cumulative flow curve
      plotly::add_trace(x=~plotDateWY,
                        y=~cumulativeFlow,
                        type="scatter",
                        mode="lines",
                        line=list(color="darkgray"),
      ...
      plotly::add_trace(data=inv_abundance_case2%>%
                          dplyr::filter(siteID==siteList[s]
                                        &waterYear==waterYears[wy]),
                        x=~plotDateWY,
                        y=~abun_M2_sum_mean,
      ...
      plotly::add_trace(data=inv_richness_case2%>%
                          dplyr::filter(siteID==siteList[s]
                                        &waterYear==waterYears[wy]),
                        x=~plotDateWY,
                        y=~mean,
  ...
  legend_items <- c(
    list(htmltools::tags$button(
      ...
      htmltools::HTML("<span style='display:inline-block;width:18px;border-top:3px solid darkgray;margin-right:6px;vertical-align:middle;'></span>Cumulative Flow")
    )),
    lapply(abun_labels, function(lbl) {
      htmltools::tags$button(
        ...
        class = "case2-legend-item active",
        `data-trace-name` = lbl,
        htmltools::HTML(paste0("<span style='display:inline-block;width:18px;border-top:3px solid darkorange;margin-right:6px;vertical-align:middle;'></span>", lbl))
      )
    }),
    lapply(rich_labels, function(lbl) {
      htmltools::tags$button(
        ...
        class = "case2-legend-item active",
        `data-trace-name` = lbl,
        htmltools::HTML(paste0("<span style='display:inline-block;width:18px;border-top:3px solid darkgreen;margin-right:6px;vertical-align:middle;'></span>", lbl))
      )
    })
  )
  ...
  curr_plot <- htmltools::tagList(
    ...
    htmltools::tags$script(htmltools::HTML(legendSyncJs))
  )

  # Save HTML plot to local documents folder
  output_file <- file.path(path.expand("~"), paste0(siteList[s], "_cumSumWY.html"))
  htmltools::save_html(
    htmltools::browsable(curr_plot),
    file = output_file,
...
<div class="figure" style="text-align: center">
<img src="https://raw.githubusercontent.com/znickerson8/NEON-Data-Skills/main/graphics/ais-aos/SYCA_stackedCumSum.png" alt=" "  />
<p class="Effects of Drying on Macroinvertebrates at D14-SYCA">Snapshot of the interactive HTML plot generated in the code above using the `plotly` package. When running the code, the plot should be saved to your 'Documents' folder.</p>
</div>
