...
This tutorial explores hydrologic data products published by NEON across
observational, instrumented, and remote sensing subsystems. It focuses on
downloading and exploring various NEON hydrologic data products. It also dives
deeper into NEON hydrology data, using co-located terrestrial-aquatic sites to
examine the water cycle at multiple levels. The code for this tutorial is
...
```{python p-import, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
import os
import pandas as pd
...
```{python p-envvar, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
...
```{python p-download-is, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Download precipitation data for a single water year
...
```{python p-files-is, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Get all file names in the download package
ptp_py.keys()
```
...
-   `science_review_flags_00045`: Lists each science review flag (SRF) date 
    range, flag value, and justification applied to the flag included in this 
    download.
...
```{python p-tables-is, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Print the first 5 records in the time series data
print("First 5 rows of TIPPRE_30min")
...
```{python p-variables-is, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# View variables file to understand data table structure
...
```{python p-download-os, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Download stable isotopes data for a single water year
...
```{python p-files-os, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Get all file names in the download package
asi_py.keys()
```
...
```{python p-tables-os, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Print the first 5 records in H2O stable isotope lab data
print("First 5 rows of asi_externalLabH2OIsotopes")
...
```{python p-variables-os, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# View variables file to understand data table structure
...
```{python p-download-geo, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Download stream morphology data for the lifetime of a site
# URL to download geospatial data from the cloud is stored in geo_surveySummary
...
```{python p-download-sae, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Download eddy covariance data for a single water year
...
```{python p-download-wc, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Download precipitation data for a single water year
ptp_py = nu.load_by_product(dpid="DP1.00045.001",
                            site="ORNL", # Terrestrial data product
                            startdate="2023-10",
...
```{python p-merge-wc, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# In this download, there are 3 well locations that publish elevation
# There is only 1 location for discharge
...
```{python p-plot-wc, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Create figure
...
```{python p-plot-wc-2, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Plot cumulative precipitation & discharge together using ggplot with 2 y-axes
...
```{python p-plot-wc-3, eval=FALSE, results="hide", message=FALSE, warning=FALSE}
# Plot scatterplots of one variable to another to assess correlation
# Create a continuous color scale by date to add the time-of-year dimension
