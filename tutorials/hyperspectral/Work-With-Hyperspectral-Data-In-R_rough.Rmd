...
f <- paste0(wd,"NEON_D17_SJER_DP3_257000_4112000_reflectance.h5") ## testing compatability with data downloaded from portal

# look at the HDF5 file structure 
View(h5ls(f,all=T)) ## should remove 'View()' for better formatting? Or so it converts better?

```

##This all needs to be changed and updated for the new file format
# Note all the metadata and different groups-

# coordinate system group - these two we will use
# reflectance group
# processing logs if you want to dig into the nitty gritty
...
HDF5 file. Let's read in the wavekengths of the band centers:
...
Band 19 has a associate wavelength center of 471.6872 nanometers (nm) which is 
...
# I think this is no longer correct with new funciton? Or at least how the data are now organized. 
# h5read() read in as bands, rows, columns (or columns, rows, it's hard to tell when they're both 1,000)
...
b34<- h5read(f,"/SJER/Reflectance/Reflectance_Data",index=list(34,1:nCols,1:nRows)) 
# what type of object is b34?
class(b34)

# Use h5closeAll() to close the open connection - this saves memory and prevents problems!
h5closeAll()
...
# View higher values
hist(b34, breaks=40,col="darkmagenta",xlim = c(5000, 15000),ylim=c(0,2000))
...
<br>
`"UTM,  1.000,  1.000,       257000.00,       4113000.0,       1.0000000,       1.0000000,  11,`  `North,  WGS-84,  units=Meters, 0" `. 
</br>
...
mapAttr<-h5readAttributes(f,"/SJER/Reflectance/Metadata/Coordinate_System/Map_Info")
mapAttr

mapInfo<-h5read(f,"/SJER/Reflectance/Metadata/Coordinate_System/Map_Info")
...
# Extract the CRS from the h5 dataset
...
res <- reflInfo$Spatial_Resolution_X_Y[1] #pulling just the X resolution because X and Y are equal
res
...
```{r}
aPixel_soil<- h5read(f,"/SJER/Reflectance/Reflectance_Data",index=list(NULL,54,35) )

h5closeAll()

plot(aPixel_soil)

aPixel_vegetation<- h5read(f,"/SJER/Reflectance/Reflectance_Data",index=list(NULL,100,35) )
plot(aPixel_vegetation)
```
