...
We'll also set a working directory for data downloads (adapt the 
working directory path for your system) and load our API token.
...
```{r veglist, results="hide", message=FALSE}
...
veg2017 <- veg |>
  filter(eventID.y=="vst_WREF_2017")

veg2017.075 <- veg2017 |>
  filter(plotID=="WREF_075")

symbols(veg2017.075$adjEasting, 
        veg2017.075$adjNorthing, 
        circles=veg2017.075$stemDiameter/100/2, 
        xlab="Easting", 
        ylab="Northing", 
        inches=F)
...
symbols(veg2017.075$adjEasting, 
        veg2017.075$adjNorthing, 
        circles=veg2017.075$adjCoordinateUncertainty, 
        inches=F, add=T, 
        fg="lightblue")
...
```{r get-chm, results="hide", message=FALSE}

byTileAOP(dpID="DP3.30015.001", site="WREF", year=2017, 
          easting=veg2017.075$adjEasting, 
          northing=veg2017.075$adjNorthing,
...
vegsub <- veg2017 |>
  filter(adjEasting >= ext(chm)[1] &
         adjEasting <= ext(chm)[2] &
         adjNorthing >= ext(chm)[3] & 
         adjNorthing <= ext(chm)[4])
...
```{python p-buffer-chm, message=FALSE, warning=FALSE}
...
Start by sorting the vegetation structure data by height.
