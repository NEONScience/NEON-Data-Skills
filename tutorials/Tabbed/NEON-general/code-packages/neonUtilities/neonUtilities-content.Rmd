...
<a href="https://github.com/NEONScience/NEON-utilities/blob/main/cheat-sheet-neonUtilities.pdf" target="_blank">neonUtilities cheat sheet</a>.
...
data. For functions that download AOP data, see the `byFileAOP()` and 
`byTileAOP()` sections in this tutorial. For SAE data, use `zipsByProduct()` 
and then `stackEddy()`, and see the <a href="https://www.neonscience.org/resources/learning-hub/tutorials/eddy-data-intro" target="_blank">flux data tutorial</a>.
...
* `cloud.mode`: Can be set to True if you are working in a cloud 
...
files and not on NEON data stored in other formats (HDF5, etc). 
...
stackByTable(filepath="~data/NEON_temp-air-single.zip")
...
# Modify the file path to the file location on your computer
nu.stack_by_table(filepath="/data/NEON_temp-air-single.zip")
