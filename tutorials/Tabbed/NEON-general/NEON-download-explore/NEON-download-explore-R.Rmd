---
title: Download and explore NEON tutorial, R only
...
data documentation. We will explore data of 3 different types, and make a 
simple figure from each.
...
2. Observational (OS) - Data collected by a human in the field, or in an 
analytical laboratory, e.g. beetle identification, foliar isotopes
3. Instrumentation (IS) - Data collected by an automated, streaming sensor, e.g. 
...
* For R users, we recommend using R version 4+ and RStudio (v 2025.05).
...
* <a href="https://github.com/NEONScience/NEON-utilities/blob/main/cheat-sheet-neonUtilities.pdf" target="_blank">neonUtilities cheat sheet</a>. A quick reference guide for users. Focuses on the R package, but applicable to Python as well.
...
and download some data! To follow the tutorial exactly, download the basic 
package of Photosynthetically active radiation (PAR) (DP1.00024.001) data 
from September-November 2019 at Wind River Experimental Forest (WREF). The 
...
# You may need to change this for Windows
# Mac:
# stackByTable("~/Downloads/NEON_par.zip")
# show the full path
# print(path.expand("~/Downloads"))

# Windows
rootPath <- Sys.getenv("USERPROFILE")
# print(root_path)
parPath <- file.path(rootPath,"Downloads","NEON_par.zip")
# print(parPath)
stackByTable(parPath)
...
par30 <- readTableNEON(
  dataFile=file.path(rootPath,"Downloads","NEON_par","stackedFiles","PARPAR_30min.csv"), 
  varFile=file.path(rootPath,"Downloads","NEON_par","stackedFiles","variables_00024.csv"))
head(par30)
...
To get the same PAR data we worked with above, we would run the line of code 
below using `loadByProduct()`.
...
* `token`: NEON API token. See <a href="https://www.neonscience.org/resources/learning-hub/tutorials/neon-api-tokens-tutorial" target="_blank">this tutorial</a> 
...
are cloudier than others.

### Plot PAR data at different tower levels
...
* `dpID`: the data product ID, e.g. DP3.30001.001
...
```{r find-tif-files, eval=TRUE}

chmFiles <- list.files(
  path = file.path(rootPath,"Downloads","DP3.30015.001"),
  pattern = "\\.tif$",
  full.names = TRUE,
  recursive = TRUE
)
...
chmPath <- file.path(rootPath,"Downloads","DP3.30015.001","neon-aop-products","2017","FullSite","D16","2017_WREF_1","L3","DiscreteLidar","CanopyHeightModelGtif")

print(chmPath)

chm <- rast(file.path(chmPath,"NEON_D16_WREF_DP3_580000_5075000_CHM.tif"))
...
plot(chm, col=topo.colors(7))

```

Now we can see canopy height across the downloaded tile; the tallest trees are 
over 60 meters, not surprising in the Pacific Northwest. There is a clearing or 
clear cut in the lower right quadrant.
...
the <a href="https://www.neonscience.org/resources/learning-hub/tutorials/get-started-neon-data-series-data-tutorials" target="_blank">Getting Started with NEON Data</a> tutorial series.
