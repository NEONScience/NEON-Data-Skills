...
* For Python users, we recommend using Python 3.10+.
...
* <a href="https://github.com/NEONScience/NEON-Utilities" target="_blank">GitHub repository for neonUtilities R package</a>
...
and download some data! You will need to create a free account to download data, 
but you do not need an account to explore the data products and documentation 
on the portal.

To follow the tutorial exactly, download 
...
for downloading data programmatically.
...
Just as the data portal requires a free account to download data, accessing data 
via the API requires an API token, which you can get through your data portal 
account. See the <a href="https://www.neonscience.org/resources/learning-hub/tutorials/api-token-setup" target="_blank">Set up a NEON API token</a> 
tutorial for instructions on getting a token and setting it up in your code environment.

Now let's use `loadByProduct()` to get the same PAR data we worked with above.

### R

Retrieve your API token and assign it to an object called `token`, which you 
will use throughout this tutorial when downloading data. If you stored your 
token in a different way (i.e., not as an environment variable), modify the 
code below to match how you stored it.

```{r get-token, results="hide"}
...
```{python p-get-token, results="hide"}
