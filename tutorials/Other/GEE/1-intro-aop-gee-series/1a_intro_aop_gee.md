...
packagesLibraries: 
topics: lidar, hyperspectral, camera, remote-sensing, canopy-nitrogen
languageTool: GEE, JavaScript
dataProduct: DP3.30006.001, DP3.30006.002, DP3.30010.001, DP3.30015.001 DP3.30024.001, DP3.30018.002
code1: 
...
NEON is planning to add the full archive of AOP L3 <a href="https://data.neonscience.org/data-products/DP3.30006.002" target="_blank">Surface Bidirectional Reflectance</a>, <a href="https://data.neonscience.org/data-products/DP3.30024.001" target="_blank">LiDAR Elevation</a>, <a href="https://data.neonscience.org/data-products/DP3.30015.001" target="_blank">Ecosystem Structure (CHM)</a>, <a href="https://data.neonscience.org/data-products/DP3.30010.001" target="_blank">High-resolution orthorectified camera imagery</a> and <a href="https://data.neonscience.org/data-products/DP3.30018.002" target="_blank">Canopy nitrogen</a> (a subset of data newly available as of Feb 2026). Since the L3 <a href="https://data.neonscience.org/data-products/DP3.30006.001" target="_blank">Surface Directional Reflectance</a> is planned to be replaced by the bidirectional (Bidirectional Reflectance Distribution Function (BRDF) and topographic corrected) reflectance as that is processed and becomes available, we are only adding directional reflectance data to GEE upon request. As of March 2026, bidirectional data is only available for AOP data collected between 2022-2025, but re-processing of older AOP data (2013-2021) will begin in Spring 2026. Please see the tutorial <a href="https://www.neonscience.org/resources/learning-hub/tutorials/neon-brdf-refl-h5-py" target="_blank">Introduction to Bidirectional Hyperspectral Reflectance Data in Python</a> for more information on the differences between the directional and bidirectional reflectance data products.

Not all of the data that is available on the NEON Data Portal is currently available in GEE, so this tutorial shows you how to find which data are currently available in GEE. If there are certain NEON sites and years of data you would like to see added to Google Earth Engine sooner, use the <a href="https://www.neonscience.org/about/contact-us" target="_blank">NEON Contact Us</a> form to request this, and include "Google Earth Engine Remote Sensing Data" in the text. 
...
```javascript
// read in the AOP image collections
...
var cnc = ee.ImageCollection('projects/neon-prod-earthengine/assets/CNC/002')
```
...
```javascript
// list available images in the NEON Surface Directional Reflectance Image Collection:
print('Images in the NEON Directional Refl Collection:',refl001.aggregate_array('system:index'));
      
// list available images in the NEON Surface Bidirectional Reflectance Image Collection:
print('Images in the NEON Bidirectional Refl Collection:',refl002.aggregate_array('system:index'));

// list available images in the NEON DEM image collection:
print('Images in the NEON DEM Collection:',dem.aggregate_array('system:index'));

// list available images in the NEON CHM image collection:
print('Images in the NEON CHM Collection:',chm.aggregate_array('system:index'));

// list available images in the NEON RGB image collection:
print('Images in the RGB Camera Collection:',rgb.aggregate_array('system:index'));

// list available images in the NEON Canopy Nitrogen (CNC) image collection:
print('NEON Images in the Canopy Nitrogen Collection',cnc.aggregate_array('system:index'));
```

In the **Console** tab to the right of the code, you will see a list of all available images. Expand each List to see the data available for each Image Collection. The names of the all the images follow the format `YEAR_SITE_#`, so you can identify the site and year of data this way. The number at the end is the Visit #; AOP typically visits each site 3 out of every 5 years, so the visit number indicates the cumulative number of times AOP has visited that site. Occasionally, AOP may re-visit a site twice in the same year. Note that the Canopy Nitrogen Image Collection only has a small number of images, this is because the dataset is newly available (first published in February 2026) and only a subset of data were published initially, to provide some initial examples for the community to review. Please see the NEON Data Notification: <a href="https://www.neonscience.org/impact/observatory-blog/new-data-product-canopy-nitrogen-subset-data-available-community-review" target="_blank">New Data Product: Canopy Nitrogen, Subset of Data Available for Community Review</a> for more details. 
...
```javascript
// Directional Reflectance (HSI_REFL/001)
...
var clbj2021_refl001_props = refl001_CLBJ_2021.toDictionary();
...
When working with NEON data, whether downloaded from the Data Portal or on GEE, we always recommend checking whether the data are Provisional or Released, and also checking the release tag of the data. On GEE, this information is included in the image properties `PROVISIONAL_RELEASED` and `RELEASE_YEAR`. If the data is released, the property `RELEASE_YEAR` will display the year of the release. For data that has been released, the Digital Object Identifier (DOI) is also available in the `DOI` tag, and for all data there is a `CITATION` tag providing a suggested citation. The code chunk below shows how to display the release, DOI, and citation information for the CLBJ 2021 directional reflectance data. 
...
var clbj2021_refl001_release_status = clbj2021_refl001_props.select(['PROVISIONAL_RELEASED']);
print('CLBJ 2021 Directional Reflectance Release Status:', clbj2021_refl001_release_status)

var clbj2021_refl001_release_year = clbj2021_refl001_props.select(['RELEASE_YEAR']);
print('CLBJ 2021 Directional Reflectance Release Year:', clbj2021_refl001_release_year)

// digital object identifier (DOI)
var clbj2021_refl001_doi = clbj2021_refl001_props.select(['DOI']);
print('CLBJ 2021 Directional Refl DOI:', clbj2021_refl001_doi)

// citation - see https://www.neonscience.org/data/guidelines-policies/citing
var clbj2021_refl001_citation = clbj2021_refl001_props.select(['CITATION']);
...
// display the bidirectional reflectance image properties
var clbj2022_refl002_props = refl002_CLBJ_2022.toDictionary()
print('CLBJ 2022 Bidirectional Reflectance Properties:',clbj2022_refl002_props);
```

<figure>
	<a href="https://raw.githubusercontent.com/NEONScience/NEON-Data-Skills/main/graphics/aop-gee2023/1a_intro_aop_gee/clbj_refl002_properties.png">
	<img src="https://raw.githubusercontent.com/NEONScience/NEON-Data-Skills/main/graphics/aop-gee2023/1a_intro_aop_gee/clbj_refl002_properties.png" alt="CLBJ 2022 Bidirectional Reflectance Properties." style="max-width: 100%; height: auto;">
	</a>
</figure>

You'll notice that there are fewer properties for this bidirectional reflectance (`HSI_REFL/002`) image than we saw for the directional reflectance (`HSI_REFL/001`) data. The data are `PROVISIONAL`, and there are no `RELEASE_YEAR` or `DOI` properties. Bidirectional reflectance data will remain provisional in 2026, since it is a newer data product (as of 2024), and is planned to be incorporated into the upcoming RELEASE-2027.

**Tip:** If your code has any errors they will display in the **Console** tab in red. For example, if you try to print out a property that does not exist (e.g. `DOI` and `RELEASE_YEAR` will not exist for any provisional data), then you will see an error in the console, as shown below. If your code is not running as expected, errors displayed in the Console can be helpful for troubleshooting, as they will tell you how and where your code failed. Print statements throughout the code can also be helpful.
...
<figure>
	...
	<img src="https://raw.githubusercontent.com/NEONScience/NEON-Data-Skills/main/graphics/aop-gee2023/1a_intro_aop_gee/console_error.png" alt="Example Console Error" style="max-width: 100%; height: auto;">
	</a>
</figure>

Finally, let's add the bidirectional reflectance RGB (true color) image to the Map as a new layer.

```javascript
// Add the Bidirectional Reflectance True Color (RGB) Image to the Map
// select the red, green, and blue bands
var refl002_CLBJ_2022_RGB = refl002_CLBJ_2022.select(['B053', 'B035', 'B019']);

// add this RGB layer to the Map
...
<a href="https://code.earthengine.google.com/55ef5123e508394b5c4c1f3b2594c1e0" target="_blank">Into to AOP GEE Image Collections</a>
