...
Visit the [NEON Data Portal landing page for Continuous
discharge](https://data.neonscience.org/data-products/DP4.00130.001) for
an availability chart showing all the sites for which Continuous
discharge is published and the period of record for each site.
...
# Add in the 15-minute table because it is not yet published -- change to your local file path if you have this repo cloned
csd$csd_15_min <- read.csv("C:/Users/nickerson/Documents/GitHub/NEON-Data-Skills/tutorials-in-development/R/aquatics/L4-Discharge-Series/continuous-discharge-basic/csd_15_min.csv", 
                           encoding = "UTF-8",
                           header = T)
csd$csd_15_min$startDateTime[nchar(csd$csd_15_min$startDateTime)==10] <- paste(csd$csd_15_min$startDateTime[nchar(csd$csd_15_min$startDateTime)==10],"00:00:00")
csd$csd_15_min$endDateTime[nchar(csd$csd_15_min$endDateTime)==10] <- paste(csd$csd_15_min$endDateTime[nchar(csd$csd_15_min$endDateTime)==10],"00:00:00")
csd$csd_15_min$startDateTime <- as.POSIXct(csd$csd_15_min$startDateTime,tz="UTC")
csd$csd_15_min$endDateTime <- as.POSIXct(csd$csd_15_min$endDateTime,tz="UTC")
```
...
-   `csd_15_min`: Continuous discharge data averaged to 15-minute
    intervals. This table began being published in RELEASE-2026 and is
    available, for a given site, starting with the earliest date of
    corrected data.
-   `csd_continuousDischarge`: Instantaneous (1-minute) continuous
    discharge data. Beginning with RELEASE-2026, this table is available
    only before the earliest date of data correction for a given site.
...
published in the "Documentation" section of the [NEON Data Portal
Landing Page](https://data.neonscience.org/data-products/DP4.00130.001)
...
| csd_15_min | 15 minute | if `release` = RELEASE-YYYY, data processed within new pipeline; data corrections applied |
...
using an executable (BaM) first developed and published by [Le Coz et
al. in 2014](https://doi.org/10.1016/j.jhydrol.2013.11.016). The
continuous stage series and inputs from the Stage-discharge rating
