...
* NEON (National Ecological Observatory Network). Fish electrofishing, gill netting, and fyke netting counts (DP1.20107.001), RELEASE-2026. https://doi.org/10.48443/s1e6-df79. Dataset accessed from https://data.neonscience.org/data-products/DP1.20107.001/RELEASE-2026 on `r format(Sys.Date(), "%B %d, %Y")`.
...
Many analyses of NEON data will require the joining of two or more data tables that contain different sets of information. For instance, the information about individual fish captures and samples from a bout is found in the `fsh_perFish` table, while information about additional fish that were not processed individually and instead counted in bulk is found in the `fsh_bulkCount` table. These two tables should be joined so that capture data can be easily summarized by pass.  
...
# Extract information about pass and sampler type from the per pass data
