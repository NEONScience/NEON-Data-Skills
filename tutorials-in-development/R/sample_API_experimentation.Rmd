...
root <- loadByProduct(dpID="DP1.10067.001",
                      site="NIWO",
                      release="RELEASE-2022",
                      check.size=F)
...
Why are there some disconnected samples?
...
nodes <- data.frame(matrix(unique(mos.samp$id), ncol=1))
colnames(nodes) <- "id"
...
root.samp <- getSampleTree("BBC.NIWO041074.20190813.NORTH")

root.samp$id <- root.samp$sampleUuid
root.samp$group <- root.samp$sampleClass
nodes <- data.frame(unique(cbind(root.samp$id, root.samp$group)))
colnames(nodes) <- c("id","group")

edges <- root.samp[,c("sampleUuid","parentSampleUuid")]
colnames(edges) <- c("to", "from")
...
rpt.samp <- getSampleTree("KONZ.20200716.R4690.B")

nodes <- data.frame(unique(cbind(rpt.samp$sampleUuid, rpt.samp$sampleClass,
                                 rpt.samp$sampleTag)))
colnames(nodes) <- c("id","group","label")

# rename classes
nodes$group <- c("Mammal", "Blood pathogen test", "Ear tissue pathogen test", 
                 "Blood sample", "Ear tissue sample",
                 "Fecal archive sample", "Hair archive sample",
                 "Blood archive sample", "Fecal archive sample",
                 "Hair archive sample", "Blood archive sample")

edges <- rpt.samp[,c("sampleUuid","parentSampleUuid")]
colnames(edges) <- c("to", "from")
