...
def list_all_available_sites(product):
    """
    list_all_available_sites lists all the available sites for a given product
    --------
     Inputs:
    ...
    for i in range(len(r.json()['data']['siteCodes'])):
        site_code = r.json()['data']['siteCodes'][i]['siteCode']
        site_codes.append(site_code)

    return site_codes

def list_all_available_urls(product):
    """
    list_all_available_urls lists all the available api url for a given product
    --------
     Inputs:
         product: the data product code (eg. 'DP3.30015.001' - CHM)
    --------
    ...
    data_urls = []
    for i in range(len(r.json()['data']['siteCodes'])):
        data_urls_temp = r.json()['data']['siteCodes'][i]['availableDataUrls']
...
def list_available_urls(product,site):
    """
    list_available_urls lists the api url for a given product and site
    --------
     Inputs:
         product: the data product code (eg. 'DP3.30015.001' - CHM)
...
def download_urls(url_list,download_folder_root,zip=False,token=False):
    ...
    for url in url_list:
        ...
        if token:
            ...
            print('WARNING! No API token entered, to improve download performance, include your API token.')
            r=requests.get(url)
...
def download_file(url,filename,token=False):
    print('token:',token)
    if token:
        r = requests.get(url+'&apiToken='+token)
    else:
...
def download_aop_files(product,site,year=None,download_folder='./data',match_string=None,check_size=True,token=False):
    """
        ...
        for i in range(len(files)):
            if match_string is not None:
                if match_string in files[i]['name']:
                    print('downloading ' + files[i]['name'] + ' to ' + download_folder)
                    try:
                        download_file(files[i]['url'],os.path.join(download_folder,files[i]['name']),token)
...
def download_all_aop_files(product,year=None,download_folder='./data',match_string=None,check_size=True,token=False):
    """
    download_aop_files downloads NEON AOP files from the AOP for a given data product, site, and 
    optional year, download folder, and 
    --------
    ...
    #get a list of the urls for a given data product, site, and year (if included)
    # if year is not None:
        # urls = list_all_available_urls_by_year(product,year)
    # else:
    urls = list_all_available_urls(product)
    
    if year != None:
        urls = [url for url in urls if year in url]
    ...
    # get the size of all the files you are planning to download
    # the get_file_size function prints the size
    size = get_file_size(urls,match_string)
