...
You can view the code used to subset these data into a (_much_) smaller hdf5 file <a href="https://github.com/NEONScience/NEON-Data-Skills/blob/main/tutorials-in-development/Compare-CHM-pt2-viz-pointclouds/Subset_GEDI_hdf5_mini_03.R" target="_blank">here</a>.
...
This next section on downloading and working with GEDI data is loosely based on the excellent `rGEDI` package tutorial <a href="https://github.com/carlos-alberto-silva/rGEDI">posted on GitHub here</a>.
...
# These are taken directly from the CHM tile,
# but can be set to any real values of lat/long.
# Remember, GADI doesn't collect in polar areas!
...
If you are using the (very small) GEDI data subset linked at the top of this tutorial, you will skip these next few steps (with all of the code commented out). In skipping these, you will avoid a mandatory ~7Gb download. However, this subset will only work with the one CHM tile that we downloaded above. For any other location, you will need to uncomment the lines of code below and that bounding box information that we just defined as an input to the `gedifinder()` funciton.
...
Next, we use the rGEDI package to read in the GEDI data. You will need to update your filepath if you downloaded a full GEDI orbit. First, we need to make a 'gedi.level1b' object using the `readLevel1B()` function. Next, we extract the geographic position of the center of each shot from the 'gedi.level1b' object using the `getLevel1BGeo()` function, and display the head of the resulting table.
...
# buffer the GEDI shot center by a radius of 15m 
# to represent the full ~30m diameter GEDI footprint
level1bgeo_WREF_UTM_buffer=st_buffer(level1bgeo_WREF_UTM, dist=15)

# plot CHM and overlay GEDI data
plot(chm)
plot(level1bgeo_WREF_UTM_buffer, add=T, col="transparent")

# add labes with the GEDI shot_number
# note, if you are using a full GEDI orbit the shot numbers
# are very long, so you might want to just plot the last
# three digits of the shot number by setting the lables to:
# labels=substr(level1bgeo_WREF_UTM$shot_number, 15, 17)
...
Let's take a look at a waveform for a single GEDI shot. We will use the number of the shot, shown as labels in the plot above, to select a waveform of interest. In this case, let's plot the the footprint labeled '002' in the northwest of the CHM.

```{r extract-wf, warning=F}

# Extracting GEDI full-waveform for a given shot_number
wf <- getLevel1BWF(gedilevel1b,shot_number = 20)
# If using a full GEDI orbit, you can select the shot numbers by the 
# last few digits as so:
...
Note that, in many instances this method will not work well to filter out points by elevation. For example, in mountainous terrain there may be valid points on hillsides that are higher elevation than noisy points above a valley below. In these cases, it works better to remove points after reading in the LAS file by using the classification attributes. Please see [add link to lidar-VST tutorial here] for an example of how to do this.
...
Now you have learned how to download GEDI data for an area of interest, and to plot those data in 3D space along with the NEON LiDAR data that coencide with that GEDI footprint.

##Optional - NEON base plots
You may also be interested to see if any of the GEDI footprints intersect a NEON base plot, which would allow for a direct comparison of the GEDI waveform with many of the datasets which are collected within the base plots, such as the vegetation structure data product containing height, DBH, and species identification of all trees >10cm DBH. While it is statistically pretty unlikely that a GEDI footprint will intersect with your base plot of interest, it is possible that _some_ GEDI footprint will intersect with _some_ base plot in your study area, so we may as well take a look:
