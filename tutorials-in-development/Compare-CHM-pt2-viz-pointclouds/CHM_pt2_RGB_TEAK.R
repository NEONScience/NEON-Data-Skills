...
WREF_LAS=readLAS("~/Downloads/NEON_lidar-point-cloud-line_TEAK/NEON.D17.TEAK.DP1.30003.001.2018-06.basic.20210418T213049Z.RELEASE-2021/NEON_D17_TEAK_DP1_321000_4097000_classified_point_cloud_colorized.laz",
                 filter = "-drop_z_below 2050 -drop_z_above 2400")
...
veglist <- loadByProduct(dpID="DP1.10098.001", site="TEAK", check.size = F, package="basic")
...
# Use extract function like CHM vs VST tutorial (hmm, but that requires a raster not LAS object)
# Let's use the lidR cropping function within the dbh of the tree

tree_boxes=tree_boxes[!is.na(tree_boxes$height),]

boles = lasclipCircle(seg_4, tree_boxes$adjEasting, tree_boxes$adjNorthing, tree_boxes$stemDiameter/100+1)
...
unique_modes = unique_modes[!is.na(unique_modes)]
