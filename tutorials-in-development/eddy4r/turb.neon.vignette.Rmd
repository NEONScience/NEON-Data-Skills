...
# estimated using the planar fit function [CKL: what's the function name?]
# in the eddy4R.turb package. This requires a longer [CKL: how long?] 
...
for(date in Para$Flow$DateOut) {
  ...
  if(Para$Flow$Read == "hdf5") {
    ...
    for(Var in c("irgaTurb", "mfcSampTurb", "valvValiNemaTurb", "soni", "amrs")){
      ...
      # [CKL: I don't understand this step. Are there time-based flags we're discarding?]
...
# correct?]. This vignette sets Deve=TRUE.
...
for(idxAgr in 1:iter) {
  ...
  for(idxSens in names(data)) {
    ...
    # Assign units for each variable, from metadata [CKL: is this correct?]
...
# [CKL: Why? Don't we want this in the output file?]
...
eddy4R.base::wrap.hdf5.wrte.dp01(inpList = out, 
                                 ...
                                 MethUcrt = FALSE, 
                                 MethSubAgr = TRUE)
# [CKL: Error: Variable names are not consistent among inputs: data, coefPoly, unitFrom, and unitTo. Check inputs.]
