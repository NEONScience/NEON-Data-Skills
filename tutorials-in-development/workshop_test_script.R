workshop.readiness <- function() {
  ...
  # make this modifiable - type of workshop as a function input that determines list of packages needed
  # or make different scripts for different workshops?
  pkgs <- c("curl", "raster", "neonUtilities", "geoNEON", "rhdf5")
  pkg.fail <- numeric()
  ...
  if("rhfd5" %in% pkgs[pkg.fail]) {
    message('Install package rhdf5 using BiocManager::install("rhdf5")')
  }
