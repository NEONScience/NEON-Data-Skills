...
```{r obtain-data, fig.height=5, fig.width=6.5, message=FALSE}
...
for the vegetation types mean on the...***BIJAN WHERE CAN THIS BE FOUND***?

To do this we'd first want to remove the sites where there is not data and then 
plot the data. 

```{r plot-MAT-MAP, message=FALSE}
...
phenofluxsites <- phenos[flux_data==TRUE&!is.na(flux_sitenames), 
...
**BIJAN: when I do this I get 171 obs of which several comes back with a blank for Flux. This seems to be that some Flux_sitenames are blank while others are NA.  Is this something that should be fixed in the Phenocam database or something we should account for here?
...
```{r midday-list, fig.height=5, fig.width=6.5, message=FALSE}

# obtaining midday_images for dukehw
duke_middays <- get_midday_list('dukehw')

#BIJAN -- I get "unused argument (direct = FALSE)" when using your original code. However it work without the "FALSE".  Okay to remove? 
...
```{r midday-time-range, fig.height=5, fig.width=6.5, message=FALSE, eval=FALSE}

# download a subset. Example April 2018
download_dir <- download_midday_images(site = 'dukehw', # which site
																			 y = 2018, # which year
																			 months = 4, # which month
																			 download_dir = tempdir()) # where on your computer
...
BIJAN: we need some context here at the end. Regarding what was downloaded but 
I don't fully understant the last steps as it seems to only be one image? 
