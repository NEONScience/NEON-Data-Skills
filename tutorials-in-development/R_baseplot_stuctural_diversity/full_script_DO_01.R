...
calculate_boundary_metrics <- function(boundary_plot=this_boundaries_df[[i]], plots_spdf=this_plots_spdf, file_df=this_files_df){
...
build_metrics_df <- function(this_plots_spdf=base_plots_SPDF, this_coords_df=coord_df, this_files_df=files_df, this_boundaries_df=boundary_list){
...
main <- function(sitecode){
  ...
  ## Use this step to download all .laz files if needed
...
for(i in 1:length(files_df$url)){
  download.file(url = files_df$url[i], destfile = files_df$name[i])
}
