...
if args.verbose:
    print("list reflectance attribs\n", list(refl.attrs))
...
number_of_values_where_no_data = np.count_nonzero(
    reflClean == metadata['data_ignore_value'])
if args.verbose:
    print("number_of_values_where_no_data\n", number_of_values_where_no_data)

total_number_of_values_in_array = (arr_size[0] * arr_size[1] * arr_size[2])
if args.verbose:
    print("total_number_of_values_in_array\n", total_number_of_values_in_array)
...
if metadata['data_ignore_value'] in reflRaw:
    ...
    if args.verbose:
        print('% of Points with No Data: ',
              np.round(number_of_values_where_no_data * 100
                       / total_number_of_values_in_array, 1))
...
# retrieve bad band windows to excise
...
plt.title('Spectral Signature for '+sitename+' Pixel '+', '.join(
    map(str, pixel_to_plot)))
...
fig = ax.get_figure()
fig.savefig(args.figdir+'/'+sitename+'_'+'_'.join(
    map(str, pixel_to_plot))+'_plot_1_all.png')
...
fig = ax1.get_figure()
fig.savefig(args.figdir+'/'+sitename+'_'+'_'.join(
    map(str, pixel_to_plot))+'_plot_2_bad_bands_marked.png')
...
w[((w >= bbw1[0]) & (w <= bbw1[1])) |
  ((w >= bbw2[0]) & (w <= bbw2[1]))] = np.nan

# the last 10 bands sometimes have noise - best to eliminate
w[-10:] = np.nan

# print wavelength values to show that -9999 values are replaced
# with nan
if args.verbose:
    print("check for NaN replacement\n", w)
...
serc_pixel_df.to_csv(args.csvdir+'/'+sitename+'_'+'_'.join(
    map(str, pixel_to_plot))+'_cleaned.csv')
...
fig = ax2.get_figure()
fig.savefig(args.figdir+'/'+sitename+'_'+'_'.join(
    map(str, pixel_to_plot))+'_plot_3_marked_and_cleaned.png')
