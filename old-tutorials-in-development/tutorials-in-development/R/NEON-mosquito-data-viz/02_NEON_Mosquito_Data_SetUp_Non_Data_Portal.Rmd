...
This is part two of a series of tutorials that will teach you how to use NEON 
...
To complete this data you will either need to complete 
<a href="/URLHERE" target="_blank"> *Tutorial One Title*</a> where we download 
the necessary data from the NEON API or download the data packet here. 

DOWNLOAD BUTTON HERE 

#6/20: Gotta update with tutorial link before publishing.
...
into five tables:
 * **mos_trapping**: field collection data. Note: if you download the "basic" package
 
# 6/20: 'Note:' remains unfinished. 
 
 * **mos_sorting**: sorting data
 * **mos_expertTaxonomistIDProcessed**: identification and pinning data. Note: 
in the User Guide this is simply referred to as the **mos_identification** 
 table. 
 * **mos_expertTaxonomistIDRaw**: the raw identification and pinning data 
 * **mos_archivepooling**: the pooling data for samples that are archived

Please note that if for a given month no mosquitos were collected then only the 
**mos_trapping** data will be included in the available data. 
...
# 6/20: Code chunk needs to be updated with the new link for the trapping table, which does not exist. I ran this using trap <- mos$mostrapping without issue (and the other two tables, 'sort' and 'id'), but for people using this tutorial individually from the 01 MOS tutorial, there should either be an edit to include a new .csv or new code to read in the mosquito data and get the tables from there.
...
* metadata about the subsampling event, includes:
  ...
  + `setDate`: When traps were set
  + `collectDate`: The date of trap collection
  + `archiveID`: Unique label of sample collection events
  + `subsampleID`: Unique label of subsampling collection events
...
# 5/2020: The three data tables that are supposed to be imported above are not currently located in the folder specified: "R-NEON-mosquito-data-viz". I found the old repo in GitHub and am working with the data there. It's in a retired repo called 'mosquito-intern-master'. 
...
data frame. To start, we will be extracting latitude and longitude information 
...
# Create a vector of unique plotIDs from the trap data frame to speed up the data scraping process
...
#6/20: Maybe include a note to change the code to your own wd.
...
match each plot ID in our latlon data frame to a plot ID in the trap dataset.
The merge command allows users to match rows of separate data frames by certain
...
#6/20: When I run the code I get this error: 'x' is NULL so the result will be NULLError in ans[ypos] <- rep(yes, length.out = len)[ypos] : replacement has length zero. This doesn't appear to affect running the code.
...
uniquetrap <-trap[!duplicated(trap[,1]),c("plotID","decimalLatitude","decimalLongitude", "elevation.y","nlcdClass.y")]
...
#Merge with taxonomy df for taxon rank
...
# 6/20: I believe this above paragraph means a link to part three of the series and a brief description.
