...
# 5/20: This should be updated to a full URL rathern than an 'include' link.
...
The data that we will primarily be working with in this tutorial are mosquito abundance and diversity data collected at regular intervals at NEON terrestrial field sites. 
The mosquitos are captured in CO<sub>2</sub> traps and identified to species. 
These data are available as NEON data product **DP1.10043.001**. More information
...
Here it specifies that we want information about data `products` and specifically 
...
# 5/2020: zipsByProduct can be replaced with loadByProduct, which combines zipsByProduct and stackByTable. This might change the workflow a little bit, but should save time.

What we get from this request is a "response" object. When we printed the response
object we got the URL, date of response, http status ("200" means it was a success, "400s" means there was some type of error), the file content type, and the size of the file.  
...
* metadata about the temperature data, includes:
...
data frame. The first step in our data wrangling process is to consolidate all of
...
#Creating a data frame with only the unique plotIDs and lat2 lon2 data for merging
...
that are present in the id data frame but not in the trap data frame and row-bind
...
will note that the individual count present in each observation of the id data frame is only the individual count of each subsample. So to estimate the 
...
in the id data frame where by-catch weight information is present. Then we divide
...
is because of changes in sampling design, making the observations for these 
...
will use both the 'idsmall' and the 'uniquetrap' data frames to create a data frame that captures the number of species at each site on a given date. 
...
# 6/20: If loadByProduct is used, stacking NEON data section above can be ignored.
