...
packagesLibraries: rgdal, ggplot2
...
rectangular, the pixels at the edge of the raster will have `NoData` values. 
Highlighting `NoData` values in a different color helps identify where the missing 
data is.
...
```{r histogram, fig.cap="Histogram with a good bell curve and some values clustered at zero."}
...
In `geom_histogram()`, 30 is the default number of bins for the data. We can define the number of bins by using the `bins` value. 

The shape of this histogram looks reasonable and there are likely no bad data values in this raster.
