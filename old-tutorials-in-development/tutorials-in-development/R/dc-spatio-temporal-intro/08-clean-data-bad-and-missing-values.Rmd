...
<i class="fa fa-star"></i> **Data Tip:** A typical remotely sensed image will have 
values from 0-255, representing a string of 8 binary numbers that correspond to 
...
to `NA`. The `NoData` value can be found in the data documentation or use the 
`GDALinfo` function.

``` {r assign-NA, fig.cap="Corrected histogram with a good bell curve and some values clustered at zero."}
...
``` {r bad-data, fig.cap=c("Raster with 100 cells and each cell is one of several colors that represents a number from 0 to 1","Raster with 100 cells like the previouss raster but many of the cells are empty indicating an NA value")} 
# create a raster
