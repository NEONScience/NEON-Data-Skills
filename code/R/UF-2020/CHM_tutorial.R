...
# Get UTM easting/northing for veg structure data
...
##  Extract all pixels within 'uncertainty' circle
bufferCHM <- extract(chm, cbind(vegsub$adjEasting, 
                                vegsub$adjNorthing),
                     buffer=vegsub$adjCoordinateUncertainty, 
                     fun=max)

# Plot comparison of max height from CHM with height as measured in veg str data
...
## Attempt #2 - aggregate all pixels to 10m cells and compare
## Aggregate to 10m
...
# Offset easting10 and northing10 to pixel center
vegbin$easting10 <- vegbin$easting10+5
vegbin$northing10 <- vegbin$northing10+5

# Extract pixel values again
...
## Compare tree heights with eachother
vegsub <- vegsub[order(vegsub$height, decreasing=T),]

# Mask out any trees that are likely to be 'underneath' taller trees
vegfil <- vegsub
...
# Extract CHM values from only the 'taller' trees
...
## Remove problem trees - dead, etc.
# What are different tree statuses?
unique(vegfil$plantStatus)

# Select only the 'live' ones
