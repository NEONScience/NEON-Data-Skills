...
the plot ID that goes with the data. The data we areusing today were collected 
...
* Point: denotes where the point is taken -- all are centoids
...
(not the corner) is 20m from the centoid. Overall this will create a 40 m x 40 m 
square plot. 

Units: Radius is in meters, matching the UTM CRS. If you're coorindates were in 
...
The `cbind()` fuction allows use to combine or bind together data by column. Make
sure to create the vertices in an order that makes sense. We recommend starting 
at the NE and proceding clockwise. 
...
coordinates of each vertex in the polygon - in order. It is alsways important 
...
recommend the `mapply()` fuction for your actual data processing. 
