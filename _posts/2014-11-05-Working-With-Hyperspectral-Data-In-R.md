...
<a href="http://neonhighered.org/data/SJER_140123_chip.h5" class="btn"> DOWNLOAD the NEON Imaging Spectrometer Data (HDF5) Format</a>. The data in this HDF5 file were collected over the San Joachim field site located in California (NEON Domain 17) and processed at NEON headquarters. The entire dataset can be access by request from the NEON website.  
</div>
...
Next, let's look at the metadata for the reflectance data. When we do this, take not of 1) the scale factor and 2) the data ignore value.
...
#REmove: The results of the code above creates an image that looks a bit more like the image that we want to see.

##2. Create a Georeferenced Raster
Next, we will create a raster using band 34. The raster format will allow us to deal wtih both the image stretch issue discussed above and the oreintation issue. Raster formats also support Coordinate REference Systems (CRS) which allow  us to plot our data in geographic space. 
...
#remove
	...
	# Convert the b34 matrix to a raster
	b34r <- raster((b34))
	plot(b34r)
