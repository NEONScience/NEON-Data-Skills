...
(DSM and DTM) into R to create a  canopy height model (CHM) and then compare 
LiDAR derived height (CHM) to field measured tree height to estimate uncertainty 
in LiDAR estimates."
...
* **rgdal:** `install.packages("dplyr")`
* **rgdal:** `install.packages("ggplot2")`
...
## Part 1: Create a LiDAR derived Canopy Height Model (CHM)
...
In this tutorial, we will create a Canopy Height Model. The 
...
# packages needed only in Part 2
library(sp)
library(dplyr)
...
Why might we have chosen these breaks? On this CHM we've marked teh locations of
the plot centers (note the black box isn't the plot boundary, but determined by the plot marker
we chose) and the locations of individual trees we measured (red overlapping circles). 

Plotting Tips: type in `help(points)` to read about the options for plotting points.
...
We could plot the easting and northing of the points, but our data doesn't yet 
...
we know these data are all in the same projection as our original CHM. Remember that 
we can quickly figure out what projection an object is in, using `object@crs`.
